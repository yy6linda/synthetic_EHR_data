...
def read_ehrdc_data(path, data_keys=None, label_keys=None, useful_keys=None, expand_labels=("person", "person_id", "death_date"),
    ...
    for k in label_keys:
        ...
        if pathlib.Path(f).exists():
            useful_keys[k] = list(set(useful_keys[k]).intersection(set(pd.read_csv(f, nrows=0).columns)))
            labels[k] = pd.read_csv(path + "death.csv", usecols=useful_keys[k])
...
def apply_indexing(data, config, key="id"):
    ...
    print("*** applying_index (data, config, key=\"id\") ---> data, dict(zip(uids_record, range(len(uids_record)))), dict(zip(uids_iter, range(len(uids_iter)))), index ***")
    print("for uids_record, type {}, length {}, this is patient dictionary mayhe".format(type(uids_record),len(uids_record)),flush = True)
    print("for uids_iter, type {}, length {} this is a concept/feature dictionary maybe ".format(type(uids_iter),len(uids_iter)),flush = True)
    print("index is a dictionary and it tells which column(condition_source_value) of which table(condition) needs to be extracted for features ", flush = True)
    print()
...
def preprocess_data(data, configs, uids=None, split_key="id"):
    ...
    return x_train, x_test, y_train, y_test, keys_train, keys_test,data_sp
...
def model_sparse_feature_cv_train(data, configs, uids=None, split_key="id"):
    ...
    if not do_cv:
        ...
        for date_lag in date_lags:
            ...
            print("data_sp shape",flush = True)
            print(data_sp.shape,flush = True)
